...
export const BulkTaskModal: React.FC<BulkTaskModalProps> = ({ parentGoal, onClose, onCreated }) => {
    ...
    return (
        <div style={{
            ...
            <div className="glass-panel" style={{ width: '600px', maxHeight: '80vh', padding: '32px', position: 'relative', overflow: 'auto' }}>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div>
                        <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem' }}>Task Name Pattern</label>
                        <input
                            value={pattern}
                    ...
                    <div>
                        <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem' }}>Number of Tasks</label>
                        <input
                            type="number"
                            value={count}
                        ...
                        <div>
                            ...
                            <div style={{
                                ...
                                {preview.map((task, idx) => (
                                    <div key={idx} style={{ padding: '4px 0', borderBottom: idx < preview.length - 1 ? '1px solid rgba(255,255,255,0.1)' : 'none' }}>
                                        {idx + 1}. {task}
                                    </div>
                                ))}
                            </div>
