...
import { api } from '../api';
// Add Edit icon
import { Trash2, TrendingUp, Calendar, Edit2, Link } from 'lucide-react';
    ...
    return (
        <>
            <div className="glass-panel" style={{ padding: '24px', position: 'relative', overflow: 'hidden' }}>
                    ...
                    <div style={{ flex: 1, display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                        <div style={{ flex: 1 }}>
                            ...
                            <div style={{ display: 'flex', gap: '8px', marginTop: '4px', flexWrap: 'wrap' }}>
                                ...
                                {goal.goalTasks && goal.goalTasks.length > 0 && (
                                    <span style={{ fontSize: '0.75rem', background: 'rgba(255,200,100,0.2)', padding: '2px 6px', borderRadius: '4px' }}>
                                        {goal.goalTasks.filter(gt => gt.task?.isCompleted).length}/{goal.goalTasks.length} tasks
                                    </span>
                                )}
                            </div>
                        </div>
                        ...
                        {summary?.mode === 'TASK_BASED' ? (
                            <>
                                <div style={{ fontSize: '1rem', color: 'var(--color-text-muted)', marginBottom: '6px' }}>
                                    Tasks complete: {summary.taskTotals.completedCount}/{summary.taskTotals.totalCount} (effort {summary.taskTotals.completedSize}/{summary.taskTotals.totalSize})
                                </div>
                            ...
                            <>
                                ...
                                <div style={{ fontSize: '0.95rem', color: 'var(--color-text-muted)', marginTop: '4px' }}>
                                    {summary ? `${summary.percentComplete.toFixed(0)}%` : ''}
                                </div>
                            </>
                        )}
                    </div>
                ...
                {/* Manual-total goals can still accept slider updates */}
                {summary?.mode === 'MANUAL_TOTAL' && (
                    <Slider
                        value={goal.currentValue}
                        max={goal.targetValue || Math.max(goal.currentValue, 1)}
